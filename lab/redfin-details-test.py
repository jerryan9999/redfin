...
'''
print get_img(html)
print get_property(html)
print get_remark(html)
for each in get_history(html):
  print each
'''
...
resultk=Queue()
with conn_mongo:
  db = conn_mongo.downloads
  for each in db['rooms'].find().limit(5):
    url = each['url']
    id = each['_id']
    print url,id
    source.put([url, id])
